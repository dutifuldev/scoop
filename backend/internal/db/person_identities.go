...
func ParseIdentityRef(raw string) (ParsedIdentityRef, error) {
	parsedURL, provider, err := parseIdentityRefURL(raw)
	if err != nil {
		return ParsedIdentityRef{}, err
	}
	providerUserID, handle, err := parseIdentityRefPath(parsedURL)
	if err != nil {
		return ParsedIdentityRef{}, err
	}
...
func parseIdentityRefURL(raw string) (*url.URL, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("invalid identity ref: %w", err)
	}
	if strings.ToLower(parsedURL.Scheme) != "id" {
		return nil, "", fmt.Errorf("identity ref must use id://")
	}
	provider := strings.ToLower(strings.TrimSpace(parsedURL.Host))
	if !providerKeyPattern.MatchString(provider) {
		return nil, "", fmt.Errorf("provider must match [a-z][a-z0-9-]*")
	}
	return parsedURL, provider, nil
}

func parseIdentityRefPath(parsedURL *url.URL) (*string, *string, error) {
	kind, value, err := identityRefKindAndValue(parsedURL)
	if err != nil {
		return nil, nil, err
	}
	switch kind {
	case "id":
		handle, err := optionalIdentityRefHandle(parsedURL)
		return stringPtr(value), handle, err
	case "handle":
		handle, err := normalizeIdentityHandle(value)
		if err != nil {
			return nil, nil, err
		}
		return nil, &handle, nil
	default:
		return nil, nil, fmt.Errorf("identity ref kind must be id or handle")
	}
}

func identityRefKindAndValue(parsedURL *url.URL) (string, string, error) {
	segments := strings.Split(strings.Trim(parsedURL.EscapedPath(), "/"), "/")
	if len(segments) != 2 {
		return "", "", fmt.Errorf("identity ref path must be /id/<value> or /handle/<handle>")
	}
	...
	if value == "" || strings.ContainsAny(value, " \t\r\n") {
		return "", "", fmt.Errorf("identity value must be non-empty and contain no whitespace")
	}
	return strings.ToLower(strings.TrimSpace(segments[0])), value, nil
}

func optionalIdentityRefHandle(parsedURL *url.URL) (*string, error) {
	rawHandle := parsedURL.Query().Get("handle")
	if rawHandle == "" {
		return nil, nil
	}
	handle, err := normalizeIdentityHandle(rawHandle)
	if err != nil {
		return nil, err
	}
	return &handle, nil
...
func (p *Pool) UpsertPersonIdentity(ctx context.Context, rawIdentityRef string, now time.Time) (*PersonIdentityRecord, error) {
	...
	err := p.gdb.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var err error
		identity, err = p.upsertPersonIdentityTx(ctx, tx, rawIdentityRef, now)
		return err
	})
	if err != nil {
		return nil, err
...
func (p *Pool) ListPersonIdentitiesForArticleUUIDs(ctx context.Context, articleUUIDs []string) (map[string][]PersonIdentityRecord, error) {
	return listArticleRelatedRecords(
		...
		p.gdb,
		articleUUIDs,
		"a.article_uuid::text AS article_uuid, pi.person_identity_id, pi.person_identity_uuid::text AS person_identity_uuid, pi.provider, pi.provider_user_id, pi.handle, pi.avatar_url, pi.identity_ref, pi.archived_at, pi.created_at, pi.updated_at",
		[]string{
			"JOIN news.article_person_identities AS api ON api.article_id = a.article_id",
			"JOIN news.person_identities AS pi ON pi.person_identity_id = api.person_identity_id",
		},
		"pi.provider, COALESCE(pi.handle, pi.provider_user_id), pi.person_identity_id",
		"query article person identities",
		func(row articlePersonIdentityRow) string { return row.ArticleUUID },
		func(row articlePersonIdentityRow) PersonIdentityRecord { return row.personIdentityRecord() },
	)
}
...
func (p *Pool) ListPersonIdentitiesForStoryIDs(ctx context.Context, storyIDs []int64) (map[int64][]PersonIdentityRecord, error) {
	return listStoryRelatedRecords(
		...
		p.gdb,
		storyIDs,
		"sm.story_id, pi.person_identity_id, pi.person_identity_uuid::text AS person_identity_uuid, pi.provider, pi.provider_user_id, pi.handle, pi.avatar_url, pi.identity_ref, pi.archived_at, pi.created_at, pi.updated_at",
		[]string{
			"JOIN news.articles AS a ON a.article_id = sm.article_id AND a.deleted_at IS NULL",
			"JOIN news.article_person_identities AS api ON api.article_id = sm.article_id",
			"JOIN news.person_identities AS pi ON pi.person_identity_id = api.person_identity_id",
		},
		"sm.story_id, pi.provider, pi.person_identity_id",
		"query story person identities",
		func(row storyPersonIdentityRow) int64 { return row.StoryID },
		func(row storyPersonIdentityRow) PersonIdentityRecord { return row.personIdentityRecord() },
	)
}
...
func (p *Pool) SetPersonIdentityArchived(ctx context.Context, identityRefOrUUID string, archived bool, now time.Time) (*PersonIdentityRecord, error) {
	...
	err := p.gdb.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		identity, err = setPersonIdentityArchivedTx(ctx, tx, found, archived, now)
		return err
	})
	if err != nil {
		return nil, err
...
func setPersonIdentityArchivedTx(
	ctx context.Context,
	tx *gorm.DB,
	identity PersonIdentity,
	archived bool,
	now time.Time,
) (PersonIdentity, error) {
	if err := tx.Model(&identity).Updates(map[string]any{
		"archived_at": archivedTimestamp(archived, now),
		"updated_at":  now.UTC(),
	}).Error; err != nil {
		return PersonIdentity{}, fmt.Errorf("archive person identity: %w", err)
	}
	...
	if err := insertAuditEventGORM(ctx, tx, nil, personIdentityArchiveAction(archived), "person_identity", identity.IdentityRef, map[string]any{
		"identity_ref": identity.IdentityRef,
		"archived":     archived,
	}); err != nil {
...
func archivedTimestamp(archived bool, now time.Time) *time.Time {
	if !archived {
		return nil
	}
	value := now.UTC()
...
func personIdentityArchiveAction(archived bool) string {
	if archived {
		return "person_identity.archive"
	}
	return "person_identity.unarchive"
}
...
func (p *Pool) SetPersonIdentityAvatarURL(ctx context.Context, identityRefOrUUID string, avatarURL *string, now time.Time) (*PersonIdentityRecord, error) {
	...
	err := p.gdb.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		identity, err = updatePersonIdentityAvatarURLTx(ctx, tx, found, normalizeOptionalString(avatarURL), now)
		return err
	})
	if err != nil {
		return nil, err
...
func updatePersonIdentityAvatarURLTx(
	ctx context.Context,
	tx *gorm.DB,
	identity PersonIdentity,
	avatarURL *string,
	...
	if err := tx.Model(&identity).Updates(map[string]any{
		...
		return PersonIdentity{}, fmt.Errorf("update person identity avatar: %w", err)
	}
	if err := tx.Where("person_identity_id = ?", identity.PersonIdentityID).First(&identity).Error; err != nil {
		return PersonIdentity{}, fmt.Errorf("query person identity: %w", err)
	}
...
func (p *Pool) upsertPersonIdentityTx(ctx context.Context, tx *gorm.DB, rawIdentityRef string, now time.Time) (PersonIdentity, error) {
	...
	identity, found, err := findPersonIdentityForUpsert(ctx, tx, parsed)
	if err != nil {
		return PersonIdentity{}, err
	}
	if !found {
		return createPersonIdentityForUpsert(ctx, tx, parsed, now)
	}
	return updatePersonIdentityForUpsert(ctx, tx, identity, parsed, now)
}

func findPersonIdentityForUpsert(ctx context.Context, tx *gorm.DB, parsed ParsedIdentityRef) (PersonIdentity, bool, error) {
	query, err := personIdentityUpsertQuery(ctx, tx, parsed)
	if err != nil {
		return PersonIdentity{}, false, err
	}
	var identity PersonIdentity
	if err := query.First(&identity).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return PersonIdentity{}, false, nil
		}
		return PersonIdentity{}, false, fmt.Errorf("query person identity: %w", err)
	}
	return identity, true, nil
}

func personIdentityUpsertQuery(ctx context.Context, tx *gorm.DB, parsed ParsedIdentityRef) (*gorm.DB, error) {
	query := tx.WithContext(ctx).Model(&PersonIdentity{}).Where("provider = ?", parsed.Provider)
	if parsed.ProviderUserID != nil {
		return query.Where("provider_user_id = ?", *parsed.ProviderUserID), nil
	}
	if parsed.Handle != nil {
		return query.Where("provider_user_id IS NULL AND handle = ?", *parsed.Handle), nil
	}
	return nil, fmt.Errorf("identity ref must include provider user ID or handle")
}

func createPersonIdentityForUpsert(ctx context.Context, tx *gorm.DB, parsed ParsedIdentityRef, now time.Time) (PersonIdentity, error) {
	identity := PersonIdentity{
...
func updatePersonIdentityForUpsert(
	ctx context.Context,
	tx *gorm.DB,
	identity PersonIdentity,
	...
	updates := personIdentityUpsertUpdates(identity, parsed, now)
	if len(updates) == 1 {
		return identity, nil
	}
	...
	if err := tx.WithContext(ctx).Where("person_identity_id = ?", identity.PersonIdentityID).First(&identity).Error; err != nil {
		return PersonIdentity{}, fmt.Errorf("query updated person identity: %w", err)
	}
	return identity, nil
}

func personIdentityUpsertUpdates(identity PersonIdentity, parsed ParsedIdentityRef, now time.Time) map[string]any {
...
func lookupPersonIdentityGORM(ctx context.Context, tx *gorm.DB, identityRefOrUUID string) (PersonIdentity, error) {
	...
	query, err := personIdentityLookupQuery(ctx, tx, trimmed)
	if err != nil {
		return PersonIdentity{}, err
	}
	var identity PersonIdentity
...
func personIdentityLookupQuery(ctx context.Context, tx *gorm.DB, trimmed string) (*gorm.DB, error) {
	query := tx.WithContext(ctx).Model(&PersonIdentity{})
	if !strings.HasPrefix(strings.ToLower(trimmed), "id://") {
		return query.Where("person_identity_uuid = ?::uuid", trimmed), nil
	}
	...
	if parsed.ProviderUserID != nil {
		return query.Where("provider = ? AND provider_user_id = ?", parsed.Provider, *parsed.ProviderUserID), nil
	}
	return query.Where("provider = ? AND provider_user_id IS NULL AND handle = ?", parsed.Provider, *parsed.Handle), nil
}
...
type articlePersonIdentityRow struct {
	ArticleUUID             string
	PersonIdentityRowFields `gorm:"embedded"`
}

func (r articlePersonIdentityRow) personIdentityRecord() PersonIdentityRecord {
	return r.PersonIdentityRowFields.personIdentityRecord()
}
...
type PersonIdentityRowFields struct {
...
func (r PersonIdentityRowFields) personIdentityRecord() PersonIdentityRecord {
	return PersonIdentityRecord(r)
}
